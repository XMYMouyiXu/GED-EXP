...
// #endif
	...
	auto stop_option = op.add<Value<int>>("k", "stop", "\'stop the search after k iterations; if not provided, then stop after going through the whole search space", -1, &stop);
...
// 	printf(" ***\n");
	...
	if(strcmp(mode.c_str(), "pair") == 0) {
		...
		for(ui i = 0;i < queries.size();i ++) {
			...
			++ candidates_cnt;
			Timer t1;
			Application *app = new Application(verify_upper_bound, lower_bound.c_str(), stop);
