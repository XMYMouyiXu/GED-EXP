...
#-------------------------------------------------------------------------------------------------------
    def fit(self):
        ...
        torch.save(self.model.state_dict(), "./{}_model_{}".format(self.args.idx, self.args.dataset))
...
#-------------------------------------------------------------------------------------------------------
    def test(self):
        ...
        self.model.load_state_dict(torch.load("./{}_model_{}".format(self.args.idx, self.args.dataset)))
        self.model.eval()
        ...
        for graph_1 in self.testing_graphs:
            ...
            for graph_2 in self.training_graphs:
                ...
                current_error_linear = torch.nn.functional.mse_loss(pred_linear, torch.Tensor([target_linear]))
