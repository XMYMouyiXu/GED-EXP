...
from utils import load_all_graphs, load_labels, load_ged, load_my_ged
...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        #     ans.append([])
        #     for j in range(n):
        #         id_pair = (self.gid[i], self.gid[j])
        #         if not id_pair in self.ged_dict:
        #             id_pair = (self.gid[j], self.gid[i])
        #             if not id_pair in self.ged_dict:
        #                 ans[-1].append(None)
        #                 continue
        #         ans[-1].append(self.ged_dict[id_pair][0][0])
        
        # f = open("test_ged_bmao", "w")
        # for i in ans:
        #     for j in i:
        #         f.write(str(j))
        #         f.write(" ")
        ...
        # i = self.gid.index(id_pair[0])
        # j = self.gid.index(id_pair[1])
        # print(self.gn[i], self.gn[j])
        # print(self.mapping[i][j])
        # print(self.ged_dict[id_pair])
        # print(self.graphs[i])
        # print(self.graphs[j])
    ...
    def load_data(self):
        ...
        # n = len(self.graphs)
        if self.args.imdb_ged == "origin":
            load_ged(ged_dict, self.args.abs_path, dataset_name, 'TaGED.json')
        elif self.args.imdb_ged == "pyg":
            load_my_ged(ged_dict, self.graphs, "pyg")
        elif self.args.imdb_ged == "bmao":
            load_my_ged(ged_dict, self.graphs, "bmao")
    ...
    def init_graph_pairs(self):
        ...
        self.args.num_testing_graphs = self.train_num
        ...
        if self.args.imdb_ged in ["pyg", "bmao"]:
            self.testing_graphs = []
            for i in range(val_num, test_num):
                temp = []
                for j in range(val_num):
                    ...
                    if not id_pair in self.ged_dict:
                        ...
                        if not id_pair in self.ged_dict:
                            continue
                    temp.append(j)
                self.testing_graphs.append((0, i, temp))
                
            if self.args.imdb_ged in ["bmao"]:
                self.testing2_graphs = []
                for i in range(val_num, test_num):
                    temp = []
                    for j in range(val_num, test_num):
                        ...
                        if not id_pair in self.ged_dict:
                            ...
                            if not id_pair in self.ged_dict:
                                continue
                        temp.append(j)
                    self.testing2_graphs.append((0, i, temp))
    ...
    def score(self, testing_graph_set='test', test_k=0):
        ...
        # print(self.gid[0])
        # print(self.gid[560])
        # temp_data = self.pack_graph_pair((0, 0, 560))
        # print(temp_data["hb"])
        # pred, pred_ged, _ = self.model(temp_data)
        # print(pred, pred_ged)
        # exit()
        ...
        for pair_type, i, j_list in tqdm(testing_graphs, file=sys.stdout):
            ...
            for j in j_list:
                ...
                if pre_ged == torch.inf or prediction == 0:
                    continue
                ...
                if prediction is None:
                    if self.args.target_mode == "linear":
                        prediction = pre_ged / data["hb"]
                    elif self.args.target_mode == "exp":
                        prediction = torch.exp(torch.tensor([-pre_ged / data["avg_v"]]).float())
                    else:
                        assert False
                ...
                prediction_linear = round_pre_ged / data["hb"]
                target_linear = gt_ged / data["hb"]
                mse_linear.append((prediction_linear - target_linear) ** 2)
        ...
        mse_linear = round(np.mean(mse_linear) * 1000, 3)
        ...
        self.results.append(('model_name', 'dataset', 'graph_set', '#testing_pairs', 'time_usage(s/100p)', 'mse', 'mse_linear', 'mae', 'acc',
                             'fea', 'rho', 'tau', 'pk10', 'pk20'))
        self.results.append((self.args.model_name, self.args.dataset, testing_graph_set, num, time_usage, mse, mse_linear, mae, acc,
                             fea, rho, tau, pk10, pk20))
    ...
    def save(self, epoch):
        torch.save(self.model.state_dict(),
                   self.args.abs_path + self.args.model_path + str(self.args.idx) + "_" + self.args.dataset + '_' + str(epoch) + "time")

    def load(self, epoch):
        self.model.load_state_dict(
            torch.load(self.args.abs_path + self.args.model_path + str(self.args.idx) + "_" + self.args.dataset + '_' + str(epoch)))
