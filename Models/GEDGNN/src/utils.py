...
def load_my_ged(ged_dict, graphs, file_type):
    if os.path.exists("./ged_{}".format(file_type)):
        with open("./ged_{}".format(file_type), "r") as f:
    ...
    for i in range(len(test_scores)):
        for j in range(i, len(test_scores[i])):
            # if j > len(test_scores[i]):
            #     continue
            if test_scores[i][j] < 0:
                continue
            if test_scores[i][j] is None:
                continue
            if graphs[i]["n"] < graphs[j]["n"]:
                id_pair = (gid[i], gid[j])
            else:
                id_pair = (gid[j], gid[i])
            ged_dict[id_pair] = ((test_scores[i][j], 0, 0, 0), [])
