...
class GENNTrainer(object):
    ...
    def process_dataset(self):
        ...
        if self.args.dataset == "new_IMDB":
            ...
            # exit()
            num_graphs = len(self.training_graphs) + len(self.val_graphs) + len(self.testing_graphs)
            all_graphs = self.training_graphs + self.val_graphs  + self.testing_graphs
            for i in range(num_graphs):
                for j in range(i, num_graphs):
                    g1 = all_graphs[i]
                    g2 = all_graphs[j]
                    if test_scores[g1["i"]][g2["i"]] < 0:
        ...
        if os.path.exists("./test_ged_{}".format(self.args.dataset)):
            ...
            num_train_graphs = len(ori_train)
    ...
    def create_batches(self):
        ...
        for i in range(len(temp_training_graphs_1)):
            ...
            if self.nged_matrix[g1["i"], g2["i"]] < 0:
                continue
        ...
        data_list_1 = list(training_graphs_1[:self.args.batch_size])
        data_list_2 = list(training_graphs_2[:self.args.batch_size])
        ...
        for epoch in epochs:
            ...
            if epoch > 0 and (epoch + 1) % 1000 == 0 and not self.model.enable_a_star:
                ...
                if best_mse > scores['mse']:
                    torch.save(self.model.state_dict(), 'best_genn_{}_{}_time.pt'.format(self.args.dataset, self.args.gnn_operator))
                    best_mse = scores['mse']
    ...
    # def score(self, test=True, test_type="traintest"):
    #     """
    #     Scoring.
    ...
    #     scores = np.zeros((len(testing_graphs), len(other_test_set)))
    #     scores_linear = np.zeros((len(testing_graphs), len(other_test_set)))
    #     mae = np.zeros((len(testing_graphs), len(other_test_set)))
    #     ground_truth = np.full((len(testing_graphs), len(other_test_set)), 1e-10)
    #     prediction_mat = np.full((len(testing_graphs), len(other_test_set)), 1e-10)
    #     tree_size_mat = np.empty((len(testing_graphs), len(other_test_set)))
    #     time_mat = np.empty((len(testing_graphs), len(other_test_set)))

    #     n1s = np.empty((len(testing_graphs), len(other_test_set)))
    #     n2s = np.empty((len(testing_graphs), len(other_test_set)))
    ...
    #     t = tqdm(total=len(testing_graphs) * len(other_test_set), ascii=True, position=0)

    #     for i, g in enumerate(testing_graphs):
    #         data_list_1 = list([g] * len(other_test_set))
    #         data_list_2 = list(other_test_set)
    ...
    #         for start_idx in range(0, len(other_test_set), self.args.batch_size):
    ...
    #             print(prediction, target, F.mse_loss(prediction, target, reduction='none').detach().numpy())
    ...
    #             n1 = _data_list_1[0].x.shape[0]
    #             n2 = _data_list_2[0].x.shape[0]
    #             m1 = _data_list_1[0].edge_index.shape[1]
    #             m2 = _data_list_2[0].edge_index.shape[1]
                
    #             prediction_ged = torch.round(-torch.log(prediction) * 0.5 * (n1 + n2))
    #             ground_truth_ged = torch.round(-torch.log(target) * 0.5 * (n1 + n2))
                
    #             mae[i, start_idx:start_idx+self.args.batch_size] = torch.abs(prediction_ged - ground_truth_ged).detach().numpy()

                
    #             temp = max(n1, n2) + max(m1, m2)
    #             prediction_linear = prediction_ged / temp
    #             target_linear = ground_truth_ged / temp
    #             scores_linear[i, start_idx:start_idx+self.args.batch_size] = F.mse_loss(prediction_linear, target_linear, reduction='none').detach().numpy()
    ...
    #     # mae = np.mean(np.abs(np.round(prediction_ged_mat) - np.round(ground_truth_ged)))
        
    #     # print(np.round(prediction_ged_mat))
    #     # print(np.round(ground_truth_ged))
    #     # print(mae)
    #     # exit()
    #     # print(list(scores))
    ...
    #     model_error_linear = np.mean(scores_linear)
    #     mae = np.mean(mae)

    #     print("\nmse(10^-3): " + str(round(model_error * 1000, 5)))
    #     print("mse_linear(10^-3): " + str(round(model_error_linear * 1000, 5)))
    ...
    def score(self, test=True, test_type="traintest"):
        ...
        t = tqdm(total=len(testing_graphs)*len(other_test_set))
        ...
        for i, graph in enumerate(testing_graphs):
            ...
            for j, graph1 in enumerate(other_test_set):
                if self.nged_matrix[graph["i"], graph1["i"]] < 0:
                    continue
                
                if graph.x.shape[0] > graph1.x.shape[0]:
                    g1 = graph
                    g = graph1
                else:
                    g = graph
                ...
                target_ged = torch.round(-torch.log(target) * 0.5 * (g.x.shape[0] + g1.x.shape[0]))
                ...
                #       float(prediction), 
                #       float(target), 
                #       float(F.mse_loss(prediction, target, reduction='none')))
            ...
            if len(prediction_mat[i]) > 20:
                prec_at_10_list.append(calculate_prec_at_k(10, np.array(prediction_mat[i]), np.array(ground_truth[i])))
            if len(prediction_mat[i]) > 30:
                prec_at_20_list.append(calculate_prec_at_k(20, np.array(prediction_mat[i]), np.array(ground_truth[i])))
